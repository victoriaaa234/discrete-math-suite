...
def Forward_Check(proof, proof_line, proof_line_count, source_lines_target_length, proof_line_length)
	if proof_line[:reference_lines].count == proof_line_count
		source_line_nums = []
		source_lines = []
		(0..proof_line_count-1).each do |idx|
		...
		if continue
			...
			puts "ERROR: INVALID SOURCE LINES"
			return false, ""
		end
	else
		puts "ERROR: INVALID NUMBER OF REFERENCES"
		return false, ""
	end
end
...
def Check_Equals(a, b)
	if a.class == b.class
		...
		puts "ERROR: COULDN'T EVALUATE EQUIVALENCE"
		return false
	end
end
...
def Identity(proof, proof_line)
	...
	if forward_check[0]
		...
		if assumption_line[1] == :and
			if assumption_line[0] == :t
				variable = assumption_line[2]
			elsif assumption_line[2] == :t
				variable = assumption_line[0]
			else
				puts "ERROR: NO VALID TRUE FOUND FOR AND IDENTITY"
				return false
			end
		elsif assumption_line[1] == :or
			if assumption_line[0] == :f
				variable = assumption_line[2]
			elsif assumption_line[2] == :t
				variable = assumption_line[0]
			else
